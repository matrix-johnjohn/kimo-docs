...
const createCatalogues = (option = {}, vitepressConfig = {}) => {
  ...
  dirPaths.forEach((dirPath) => scannerMdFile(dirPath, option, node_path.basename(dirPath), vitepressConfig));
    ...
    if (node_fs.statSync(filePath).isDirectory()) {
      ...
      if (catalogue && path) {
        ...
        catalogueInfo.push({
          ...
          path,
          catalogues: createCatalogueList(node_path.join(srcDir, path), option, `/${path}/`, vitepressConfig)
        });
      }
    }
  });
...
const createCatalogueList = (root, option, prefix = "/", vitepressConfig = {}) => {
  ...
  const dirOrFilenames = node_fs.readdirSync(root);
  dirOrFilenames.forEach((dirOrFilename) => {
    const fileAbsolutePath = node_path.resolve(root, dirOrFilename);
    const { index: indexStr, title, name } = resolveFileName(dirOrFilename, fileAbsolutePath, indexSeparator);
    const index = parseInt(indexStr, 10);
    if (node_fs.statSync(fileAbsolutePath).isDirectory()) {
      ...
      const content = node_fs.readFileSync(fileAbsolutePath, "utf-8");
      ...
      const catalogueItem = {
        title: finalTitle,
        url: (removeMarkdownExt(rewrites[`${filePath.replace(/^\//, "")}.md`]) || filePath) + (cleanUrls ? "" : ".html"),
        frontmatter
      };
      if (isIllegalIndex(index)) catalogueItemListNoIndex.push(catalogueItem);
      else catalogueItemList[index] = catalogueItem;
